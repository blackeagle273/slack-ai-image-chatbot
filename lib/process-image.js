...
import { parseEditingCommand } from "./editing-options.js";

const openai = new OpenAI({
  apiKey: process.env.OPENAI_API_KEY,
});

export async function processImageRequest({ imageUrl, prompt, userId, channelId, app }) {
  logger.debug("processImageRequest called");
  let initialMessageTs;

  // Helper function to retry Slack API calls
  async function retrySlackApiCall(fn, retries = 3, delayMs = 1000) {
