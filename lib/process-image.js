...
async function handleOpenAIImageResponse({
  ...
  (async () => {
    ...
    const outputFilename = `gpt-generated-image.png`;
    fs.writeFileSync(outputFilename, base64String, 'base64', (error) => {
      if (error) logger.error('Error writing base64 string to file:', error);
    });
    ...
    try {
      ...
      logger.debug(`Deleted temporary file ${outputFilename}`);
    } catch (unlinkError) {
      logger.error(`Failed to delete temporary file ${outputFilename}:`, unlinkError);
    }
