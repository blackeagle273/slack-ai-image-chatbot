...
export default function registerSlackActions(app, {
  ...
  app.action('mask_checkbox', async ({ ack }) => {
    await ack();
  });
  ...
  app.action('generate_image', async ({ body, ack, client, logger }) => {
    ...
    try {
      ...
      if (
        body?.state?.values?.select_image_size_block?.select_image_size ?? null
      ) {
        selectedSize = body.state.values.select_image_size_block.select_image_size.selected_option.value;
      }

      // Determine if first image is a mask
      let isFirstImageMask = false;
      const maskBlock = body?.state?.values?.mask_checkbox_block?.mask_checkbox;

      if (
        maskBlock &&
        Array.isArray(maskBlock.selected_options) &&
        maskBlock.selected_options.length > 0
      ) {
        // Any checked option means the first image is a mask
        isFirstImageMask = true;
      }
        ...
        const updatedBlocks = msg.blocks.map((block) => {
          if (block.block_id === 'mask_checkbox_block') {
            return {
              ...
              text: {
                type: 'mrkdwn',
                text: `_${isFirstImageMask ? '"Use a mask"' : '"No mask"'} was selected._`,
              },
            }
          }
